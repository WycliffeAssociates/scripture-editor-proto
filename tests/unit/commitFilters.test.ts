...
describe("scope policies — expansion shape", () => {
  function makeScopedEvent(scope: CommitEvent["meta"]["scope"]): CommitEvent {
    return {
      meta: {
        kind: "userEdit",
        scope,
...
describe("isDirtyBufferRelevant", () => {
  ...
  it("reacts to a user edit (could make a book dirty)", () => {
    expect(
      isDirtyBufferRelevant(makeEvent("userEdit", chapterPatch, true)),
    ).toBe(true);
  });

  it("reacts to the save clean-mark (metadataOnly + bulk patch, dirtyTextContent false) so it can CLEAR a backup", () => {
    expect(
      isDirtyBufferRelevant(makeEvent("metadataOnly", bulkPatch, false)),
    ).toBe(true);
  });

  it("ignores pure selection-only commits (no state change)", () => {
    expect(
      isDirtyBufferRelevant(makeEvent("metadataOnly", selectionPatch, false)),
    ).toBe(false);
  });

  it("ignores initial load population", () => {
    expect(isDirtyBufferRelevant(makeEvent("load", bulkPatch, true))).toBe(
      false,
    );
  });

  it("reacts to imports and structural fixups (content may change dirty state)", () => {
    expect(isDirtyBufferRelevant(makeEvent("import", chapterPatch, true))).toBe(
      true,
    );
    expect(
      isDirtyBufferRelevant(makeEvent("structuralFixup", chapterPatch, true)),
    ).toBe(true);
  });
});
