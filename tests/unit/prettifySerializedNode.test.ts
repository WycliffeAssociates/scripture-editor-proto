...
describe("prettifySerializedNode utils", () => {
  ...
  describe("ensureSpaceBetweenNodes", () => {
    ...
    it("should not add space for char marker -> text boundary", () => {
      const charMarker = createToken("\\fqa", UsfmTokenTypes.marker, "fqa");
    ...
    it("should not add space for char endMarker -> punctuation boundary", () => {
      const charEnd = createToken("\\fqa*", UsfmTokenTypes.endMarker, "fqa");
  ...
  it("should handle the complex poetry example from user requirements", () => {
    ...
    const q2Index = children.findIndex(
      (n, idx) => idx > 0 && (n as SerializedUSFMTextNode).marker === "q2",
    );
