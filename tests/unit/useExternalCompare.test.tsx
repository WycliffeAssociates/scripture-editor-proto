...
function makeChapter(text: string, chapterNumber = 1): ScriptureChapterState {
  return {
    ...
    dirty: text !== "source",
    sourceTokens: [
      { kind: "text", source: "source", id: `src-${chapterNumber}` },
    ],
  ...
  const state = useExternalCompare({
    ...
    usfmOnionService: {
      ...
      diffScope: vi.fn(async (scope) =>
        scope.map(
          (entry: {
            baselineTokens: { source: string }[];
            currentTokens: { source: string }[];
          }) =>
      ...
      revertDiffBlock:
        props.revertDiffBlock ?? vi.fn(async (sourceTokens) => sourceTokens),
    } as never,
    allProjects: [],
...
describe("useExternalCompare", () => {
  ...
  it("aborts an in-flight incoming auto-accept if a save begins before it commits (gate rechecked at the mutation boundary)", async () => {
    ...
    compareSourceLoaderMock.loadRemoteLatest.mockReturnValueOnce(pendingLoad);
  ...
  it("does not clobber a concurrent commit landing during the hunk's revertDiffBlock await (no-await-between-draft-and-commit)", async () => {
    ...
    const store = renderHarness({
      ...
      revertDiffBlock: () =>
        new Promise((resolve) => {
          releaseRevert = () =>
            resolve([{ kind: "text", source: "incoming", id: "in" }]);
        }),
    });
  ...
  it("invalidates all touched chapters before refreshing after take-all", async () => {
    ...
    expect(hasDiffs(latestState?.state.diffsByChapter)).toBe(false);
    expect(compareServiceMock.buildCompareResultAsync).toHaveBeenCalledTimes(1);
  ...
  it("keeps incoming review open when the same verse already has dirty local work", async () => {
    ...
    await act(async () => {
      await latestState?.actions.openRemoteLatestReview(openDiffModal, false);
      await flush();
    });

    expect(openDiffModal).toHaveBeenCalled();
    expect(latestState?.state.diffsByChapter?.GEN?.[1]?.[0]?.semanticSid).toBe(
      "GEN 1:1",
    );
    ...
    expect(latestState?.state.pendingRemotePartialReconciliation).toMatchObject(
      { relationship: "behindOnly" },
    );
  });
  ...
  it("auto-accepts diverged incoming when 3-way changed books are disjoint", async () => {
    ...
    let result:
      | Awaited<
          ReturnType<
            ReturnType<
              typeof useExternalCompare
            >["actions"]["openRemoteLatestReview"]
          >
        >
      | undefined;
    await act(async () => {
    ...
    expect(store.read()[1]?.chapters[0]?.currentTokens[0]?.source).toBe(
      "local-exo",
    );
