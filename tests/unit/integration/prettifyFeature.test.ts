...
const createSerializedState = async (
  usfmContent: string,
...
describe("Prettify Feature Integration", () => {
  describe("Prettify Book", () => {
    it("should prettify messy USFM content in a book", async () => {
...
\\v 2 My soul makes its boast in the Lord;  let the humble hear and be glad.  \\v 3 Oh magnify the Lord with me and let us exalt his name together`,
      ...
      const originalChildren = file.chapters[0].lexicalState.root.children;
      ...
      const chapterNumberIndex = chapterChildren.findIndex(
        (node) =>
          isSerializedUSFMNumberedMarkerNode(node) && node.text.trim() === "1",
      );
      expect(chapterNumberIndex).toBeGreaterThanOrEqual(0);
      expect(chapterChildren[chapterNumberIndex + 1]?.type).toBe("linebreak");
      ...
      const nodesWithPara = (
        await createSerializedState(`\\id GEN
\\c 1
Some text
\\p Paragraph text`)
      ).root.children;
    ...
    it("should not produce root-unsafe children in usfm/plain mode", async () => {
      const ugly =
      ...
      const editor = await createTestEditor(ugly, {
        needsParagraphs: false,
      });
      const serialized = editor.getEditorState().toJSON();
      const rootChildren = serialized.root.children as SerializedLexicalNode[];
    ...
    it("bridges consecutive verse markers and strips leading enumerator in bridged text", async () => {
      const input =
      ...
      const editor = await createTestEditor(input, {
        needsParagraphs: false,
      });
      const serialized = editor.getEditorState().toJSON();
      const rootChildren = serialized.root.children as SerializedLexicalNode[];
  ...
  describe("Prettify Project", () => {
    it("should apply transformation to all files and chapters", async () => {
...
These are the   names`,
      ...
      const genChapterPara = file1.chapters[0].lexicalState.root.children.find(
        (n) =>
          n.type === USFM_PARAGRAPH_NODE_TYPE &&
          (n as { marker?: string }).marker === "c",
      );
      expect(genChapterPara).toBeTruthy();
      const genChapterChildren = (genChapterPara as { children?: unknown })
        .children as SerializedLexicalNode[];
      ...
      expect(genChapterChildren[genChapterIndex + 1]?.type).toBe("linebreak");
      ...
      const exoChapterPara = file2.chapters[0].lexicalState.root.children.find(
        (n) =>
          n.type === USFM_PARAGRAPH_NODE_TYPE &&
          (n as { marker?: string }).marker === "c",
      );
      expect(exoChapterPara).toBeTruthy();
      const exoChapterChildren = (exoChapterPara as { children?: unknown })
        .children as SerializedLexicalNode[];
      ...
      expect(exoChapterChildren[exoChapterIndex + 1]?.type).toBe("linebreak");
