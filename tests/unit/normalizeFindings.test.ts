...
describe("lintIssuesToFindings", () => {
  ...
  it("gives twins distinct ids whose set is independent of engine output order", () => {
    ...
    const forward = lintIssuesToFindings([twin(), twin()]).map((f) => f.id);
    const reversed = lintIssuesToFindings([twin(), twin()]).map((f) => f.id);
  ...
  it("carries identity fields, payload, and category; preserves input order", () => {
    ...
    expect(findings[0].source === "onion" ? findings[0].issue : null).toBe(
      issues[0],
    );
