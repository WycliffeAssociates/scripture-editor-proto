...
class MockFileSystemDirectoryHandle {
  ...
  async getFileHandle(
    ...
    if (existing) {
      throw createHandleError("TypeMismatchError", `Expected file at ${name}`);
    }
    if (options?.create) {
...
describe("OpfsGitFs", () => {
  ...
  it("supports basic sibling file writes and stats through OPFS", async () => {
    ...
    const names = await runtime.fs.promises.readdir("/appData/temp/example");
  ...
  it("works with WebGitProvider and real isomorphic-git operations", async () => {
    ...
    await runtime.fs.promises.writeFile(`${dir}/manifest.yaml`, "title: OPFS");
