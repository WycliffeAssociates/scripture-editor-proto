...
describe("matchFormattingByVerseAnchors", () => {
  it("preserves chapter markers while copying structural markers before verse 1", () => {
    ...
    expect(
      result.tokens.some(
        (token) => token.tokenType === TokenMap.marker && token.marker === "p",
      ),
    ).toBe(true);
  });
  ...
  it("copies boundary markers before the next verse anchor", () => {
    ...
    const verse2Index = result.tokens.findIndex(
      (token) => token.tokenType === TokenMap.marker && token.sid === "PSA 1:2",
    );
    const qBeforeVerse2 = result.tokens
      .slice(Math.max(0, verse2Index - 3), verse2Index)
      .some(
        (token) => token.tokenType === TokenMap.marker && token.marker === "q",
      );
  ...
  it("copies missing intra-verse markers into the target segment", () => {
    ...
    const verse1Index = result.tokens.findIndex(
      (token) => token.tokenType === TokenMap.marker && token.sid === "PSA 1:1",
    );
    const verse2Index = result.tokens.findIndex(
      (token) => token.tokenType === TokenMap.marker && token.sid === "PSA 1:2",
    );
  ...
  it("does not suggest intra-verse markers already present in target in sequence", () => {
    ...
    expect(
      result.suggestions.some(
        (suggestion) => suggestion.verse === "1" && suggestion.marker === "q2",
      ),
    ).toBe(false);
  });
  ...
  it("strips target paragraph markers and linebreaks before applying source boundaries", () => {
    ...
    expect(
      result.tokens.some(
        (token) => token.tokenType === TokenMap.marker && token.marker === "m",
      ),
    ).toBe(false);
    expect(
  ...
  it("carries source s5 markers", () => {
    ...
    expect(
      result.tokens.some(
        (token) => token.tokenType === TokenMap.marker && token.marker === "s5",
      ),
    ).toBe(true);
    expect(
      result.tokens.some(
  ...
  it("keeps boundary markers across s5 separators", () => {
    ...
    expect(
      boundaryBeforeVerse15.some(
        (token) => token.tokenType === TokenMap.marker && token.marker === "b",
      ),
    ).toBe(true);
    expect(
      boundaryBeforeVerse15.some(
  ...
  it("strictly mirrors source s5 and paragraph boundaries before the next verse", () => {
    ...
    const verse4Index = result.tokens.findIndex(
      (token) => token.tokenType === TokenMap.marker && token.sid === "REV 1:4",
    );
  ...
  it("recommended mode preserves inline markers but strips boundary-style markers", () => {
    ...
    expect(
      verse1Body.some(
        (token) => token.tokenType === TokenMap.marker && token.marker === "q2",
      ),
    ).toBe(true);
    expect(
      verse1Body.some(
