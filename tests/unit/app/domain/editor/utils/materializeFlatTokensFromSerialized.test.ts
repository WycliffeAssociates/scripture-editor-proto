...
describe("materializeFlatTokensFromSerialized", () => {
  ...
  describe("paragraph-tree input (Regular mode)", () => {
    it("emits synthetic marker then children for paragraph container", () => {
      const input = [makeParagraphContainer("p", [makeTextNode("Verse text")])];
      ...
      expect((result[1] as SerializedUSFMTextNode).text).toBe("Verse text");
    });
    ...
    it("handles multiple paragraph containers with different markers", () => {
      ...
      expect((result[0] as SerializedUSFMTextNode).text).toBe("\\p");
      expect((result[1] as SerializedUSFMTextNode).text).toBe("Normal para");
      expect((result[2] as SerializedUSFMTextNode).text).toBe("\\q1");
      expect((result[3] as SerializedUSFMTextNode).text).toBe("Poetry line");
    });
    ...
    it("preserves text content when markerText has no trailing space", () => {
      const input = [
        {
          type: USFM_PARAGRAPH_NODE_TYPE,
          id: "p1",
      ...
      expect((result[1] as SerializedUSFMTextNode).text).toBe("Heading text");
    });
  ...
  describe("nested editor nodes", () => {
    ...
    it("handles nested editors inside paragraph containers", () => {
      const nestedEditor = makeNestedEditorNode("f", [makeTextNode("Note")]);
      const input = [
  ...
  describe("walkFlatTokensSlidingWindow", () => {
    it("provides prev/curr/next for each token", () => {
      const input = [makeTextNode("A"), makeTextNode("B"), makeTextNode("C")];
    ...
    it("works with paragraph containers", () => {
      const input = [makeParagraphContainer("p", [makeTextNode("Content")])];

      const windows = [...walkFlatTokensSlidingWindow(input)];

      expect(windows).toHaveLength(2);
      // synthetic marker -> content
      expect((windows[0].curr as SerializedUSFMTextNode).text).toBe("\\p");
      expect((windows[0].next as SerializedUSFMTextNode).text).toBe("Content");
      expect((windows[1].prev as SerializedUSFMTextNode).text).toBe("\\p");
      expect((windows[1].curr as SerializedUSFMTextNode).text).toBe("Content");
    });
  });
