...
describe("translation notes contracts", () => {
  it("normalizes packed per-book TN storage without raw markdown paths", () => {
    ...
    expectTypeOf<
      keyof PackedTranslationNotesBook
    >().not.toMatchTypeOf<"relativePath">();
    expectTypeOf<
      keyof PackedTranslationNotesBook
    >().not.toMatchTypeOf<"browsePath">();
    expectTypeOf<
      keyof PackedTranslationNotesBook
    >().not.toMatchTypeOf<"documentId">();
  });
  ...
  it("pins the TN-specific read capability to packed book access", async () => {
    ...
    >();
    expectTypeOf<
      keyof PackedTranslationNotesReadable
    >().not.toMatchTypeOf<"listDocuments">();
    expectTypeOf<
      keyof PackedTranslationNotesReadable
    >().not.toMatchTypeOf<"readDocument">();
