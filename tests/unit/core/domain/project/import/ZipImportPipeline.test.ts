...
describe("ZipImportPipeline", () => {
  ...
  it("emits extract and copy progress while importing a zip archive", async () => {
    unzipMock.mockImplementation(
      (
        ...
        cb(null, {
          "project/manifest.yaml": new TextEncoder().encode("dublin_core: {}"),
  ...
  it("cleans up temp extraction and staged files when final copy fails", async () => {
    ...
    expect(removeSpy).toHaveBeenCalledWith("/appData/temp/project.zip", {
      recursive: false,
    });
  });
    ...
    unzipMock.mockImplementation(
      (
        ...
        cb(null, {
          ...
          "project/content/usfm.txt": new TextEncoder().encode("hello"),
        });
      },
    );
