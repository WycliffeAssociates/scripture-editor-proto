...
vi.mock("@tauri-apps/api/path", () => {
  ...
  return {
    ...
    dirname: vi.fn((path: string) => normalizePath(pathModule.dirname(path))),
...
describe("TauriFileSystem", () => {
  ...
  it("allows app-private cloud session files under appDataRoot", async () => {
    const fs = new TauriFileSystem(roots);
    const sessionPath = "/mock/app/local/git-remote/git-remote-session.json";
  ...
  it("preserves windows drive-letter roots when resolving managed paths", async () => {
    ...
    expect(writeTextFile).toHaveBeenCalledWith(projectInfoPath, '{"ok":true}');
  });
});
