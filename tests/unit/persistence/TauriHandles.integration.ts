...
vi.mock("@tauri-apps/api/path", () => {
    ...
    const mockAppDataRoot = pathModule.join(
        process.cwd(),
        "mock-app-local-data",
    );
    return {
        appDataDir: vi.fn(() => Promise.resolve(mockAppDataRoot)),
        appLocalDataDir: vi.fn(() =>
            Promise.resolve(mockAppDataRoot),
        ),
        join: vi.fn(pathModule.join),
        homeDir: vi.fn(() =>
            Promise.resolve(pathModule.join(process.cwd(), "mock-home-dir")),
        ),
...
describe("TauriFileHandle Integration Tests (LIVE FS I/O)", () => {
    beforeAll(async () => {
        ...
        tauriDirectoryProvider =
            await TauriDirectoryProvider.create("test-app");
...
describe("TauriDirectoryHandle Integration Tests (LIVE FS I/O)", () => {
    ...
    afterAll(async () => {
        try {
            ...
            console.log(
                `\t🧹 Cleaned up test directory for TauriDirectoryHandle: ${testRootPath}\n`,
            );
        } catch (e) {
            console.error(
                `\t🚨 Failed final cleanup for ${testRootPath}: ${e}`,
            );
        }
    });

    test("getDirectoryHandle should create and return a directory handle", async () => {
        const rootHandle = new TauriDirectoryHandle(
            testRootPath,
            tauriDirectoryProvider.getHandle.bind(tauriDirectoryProvider),
        );
        ...
        const newDirHandle = await rootHandle.getDirectoryHandle(newDirName, {
            create: true,
        });
        ...
        expect(
            actualEntries.some(
                (entry) => entry.name === newDirName && entry.isDirectory,
            ),
        ).toBe(true);
    });
    ...
    test("getFileHandle should create and return a file handle", async () => {
        ...
        const newFileHandle = await rootHandle.getFileHandle(newFileName, {
            create: true,
        });
    ...
    test("removeEntry should remove a child entry", async () => {
        ...
        const initialEntries: [string, IPathHandle][] = [];
        for await (const entry of rootHandle.entries()) {
            initialEntries.push(entry);
        }
        expect(initialEntries.some(([name]) => name === fileToRemove)).toBe(
            true,
        );

        await rootHandle.removeEntry(fileToRemove);

        const finalEntries: [string, IPathHandle][] = [];
        for await (const entry of rootHandle.entries()) {
            finalEntries.push(entry);
        }
        expect(finalEntries.some(([name]) => name === fileToRemove)).toBe(
            false,
        );

        await expect(
            readTextFile(await join(testRootPath, fileToRemove)),
        ).rejects.toThrow();
    });

    test("TauriDirectoryHandle should return its parent directory", async () => {
        const subDir1Handle = new TauriDirectoryHandle(
            testSubDirPath1,
            tauriDirectoryProvider.getHandle.bind(tauriDirectoryProvider),
        );
    ...
    test("getFileHandle should resolve relative paths with ./ correctly", async () => {
        ...
        const fileHandle = await rootHandle.getFileHandle(relativePath, {
            create: true,
        });
