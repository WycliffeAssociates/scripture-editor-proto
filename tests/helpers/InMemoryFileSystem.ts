...
function ensureParentDirectories(directories: Set<string>, path: string): void {
...
export class InMemoryFileSystem implements FileSystem {
    ...
    if (!opts?.recursive) {
      const hasChildren =
        ...
        [...this.directories].some(
          (candidate) =>
            candidate !== normalized && dirname(candidate) === normalized,
        );
      if (hasChildren) {
  ...
  async move(from: string, to: string): Promise<void> {
    ...
    const files = [...this.files.entries()].filter(
      ([filePath]) => filePath === source || filePath.startsWith(`${source}/`),
    );
  ...
  async createTempFile(prefix: string, suffix = ""): Promise<string> {
    this.tempCounter += 1;
    const path = normalizePath(`/temp/${prefix}-${this.tempCounter}${suffix}`);
