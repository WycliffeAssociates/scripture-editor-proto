...
export function parseGitRemoteProjectInfo(
  ...
  if (!parsed.success) {
    ...
    throw new Error(
      v.flatten(parsed.issues).root?.[0] ?? "Invalid git remote info record",
    );
  }
...
export function parseGitRemoteProjectStatus(
  ...
  if (!parsed.success) {
    ...
    if (
      ...
      if (record.kind === LEGACY_GIT_REMOTE_PROJECT_STATUS_SYNCING) {
        ...
        return {
          projectPath: normalizeGitRemoteProjectPath(record.projectPath),
          kind: GIT_REMOTE_PROJECT_STATUS_CONNECTED,
          lastCheckedAt:
      ...
      throw new Error(`Unsupported git remote project status: ${record.kind}`);
    }
    throw new Error(
  ...
  return {
    ...
    latestIncomingAuthorName: parsed.output.latestIncomingAuthorName ?? null,
  };
}
