...
export class ProjectRepository implements IProjectRepository {
  ...
  async loadProject(projectId: string): Promise<Project | null> {
    try {
      const userDataDir = await this.directoryProvider.getUserDataDirectory();
      const projectsRootDir = await userDataDir.getDirectoryHandle("projects", {
        create: true,
      });
      const projectDir = await projectsRootDir.getDirectoryHandle(projectId, {
        create: true,
      });
      ...
      for await (const [name, handle] of projectsDir.entries()) {
        ...
        if (handle.kind === "directory") {
          try {
            ...
            const project = await this.projectLoader.loadProject(
              directoryHandle,
              new FileWriter(this.directoryProvider, directoryHandle)
            );
            if (project) {
