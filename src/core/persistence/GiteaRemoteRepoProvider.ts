...
const GiteaRepoRecordSchema = v.object({
  ...
  // Gitea sends `parent: null` (not omitted) for every non-fork repo, so this
  // must accept null — `v.optional` alone rejects an explicit null.
  parent: v.nullish(GiteaRepoParentSchema),
});
...
export class GiteaRemoteRepoProvider implements RemoteRepoProvider {
  ...
  async listWritableRepos(args: {
    ...
    return await this.listRepoPage({
      ...
      query: { uid: userId },
      map: (repo) => mapRepoSummary(repo, args.username, args.hostBaseUrl),
      filter: (repo) =>
  ...
  private async fetchRepoRecord(args: {
    ...
    const encodedName = encodeURIComponent(args.name);
    const response = await this.fetchImpl(
      new URL(`/api/v1/repos/${encodedOwner}/${encodedName}`, args.hostBaseUrl),
      {
        method: "GET",
    ...
    while (true) {
      ...
      const filteredRecords = repoRecords.filter(
        (repo) =>
          ...
          (
            args.filter ??
            ((candidate) => isWritableRepo(candidate, args.username))
          )(repo),
      );
...
function mapRepoSummary(
  ...
  return {
    ...
    defaultBranch: repo.default_branch || REMOTE_REPO_CREATED_DEFAULT_BRANCH,
    topics: repo.topics ?? [],
    canWrite:
...
function inspectScriptureBurritoMetadata(
  ...
  const [metadata] = tryParseScriptureBurritoMetadata(JSON.parse(metadataText));
  if (!metadata) {
    throw new Error(
  ...
  return {
    ...
    isScriptureTextTranslation:
      normalizeMetadataToken(metadata.type?.flavorType?.name) === "scripture" &&
      normalizeMetadataToken(metadata.type?.flavorType?.flavor?.name) ===
        "texttranslation" &&
      normalizeMetadataToken(metadata.type?.flavorType?.flavor?.projectType) ===
        "standard",
  };
}
