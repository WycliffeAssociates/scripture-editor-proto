...
function parseVerseSegments(tokens: TokenEnvelope[]): VerseSegment[] {
  ...
  for (let i = 0; i < tokens.length - 1; i++) {
    ...
    if (!isVerseMarkerToken(marker) || !isNumberRangeToken(number)) continue;

    const parsedSid = parseSid(number.sid ?? marker.sid ?? "");
    const chapterFromSid = parsedSid?.chapter ?? 0;
    const chapter =
      chapterFromSid > 0 ? chapterFromSid : guessChapterFromTokens(tokens, i);
...
export function matchFormattingByVerseAnchors({
  ...
  for (const targetSegment of targetSegments) {
    ...
    output.push(...normalizedTargetTokens.slice(cursor, targetBoundaryStart));

    if (replacementBoundary) {
      if (
        markerSignature(targetBoundary) !== markerSignature(replacementBoundary)
      ) {
  ...
  return {
    ...
    stats: {
      ...
      sourceOnlyVerses: [...sourceKeys].filter((key) => !targetKeys.has(key))
        .length,
      targetOnlyVerses: [...targetKeys].filter((key) => !sourceKeys.has(key))
        .length,
