...
export function recoverMalformedMarkers(
  ...
  if (
    (token.tokenType === TokenMap.text || token.tokenType === TokenMap.error) &&
    ...
    if (match) {
      ...
      if (ALL_USFM_MARKERS.has(capturedMarker)) {
        ...
        const remainingText = token.text.slice(matchIndex + markerText.length);
...
function insertLinebreakAfterParaMarkersInternal(
  ...
  const isAlwaysAfter = PRETTIFY_LINEBREAK_BEFORE_AND_AFTER_MARKERS.has(marker);
...
function removeBridgeVerseEnumerators(
  ...
  for (let i = 0; i < out.length - 1; i++) {
    ...
    if (candidateVerse < range.start || candidateVerse > range.end) continue;
...
export function prettifyTokenStream(
  ...
  for (let i = 0; i < withDefaultParagraphs.length; i++) {
    ...
    withBefore.push(...insertLinebreakBeforeParaMarkersInternal(token, prev));
  }
  ...
  for (let i = 0; i < withAfter.length; i++) {
    ...
    withChapterBreak.push(
      ...insertLinebreakAfterChapterNumberRangeInternal(token, prev, next),
    );
  }
