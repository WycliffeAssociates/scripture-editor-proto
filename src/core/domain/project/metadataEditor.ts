            ...
            .find((candidate) => {
                ...
                if (
                    ...
                    candidate.identifier ===
                        args.project.identifier.toLowerCase()
                ) {
                    return true;
                }

                const pathBookCode = guessBookCodeFromPath(
                    args.project.path ?? "",
                );
                if (
                    pathBookCode &&
                ...
                const titleBookCode = guessBookCodeFromPath(
                    args.project.title ?? "",
                );
...
async function buildRcDraftProjects(args: {
    ...
    return (args.manifest.projects ?? []).map((project) => {
        ...
        return {
            ...
            suggestedPath:
                hasValidPath || !suggested ? undefined : suggested.path,
        };
    });
}
...
async function normalizeSbDraft(args: {
    ...
    return {
        ...
        meta: {
            ...
            defaultLocale: "en",
            dateCreated:
                args.draft.meta.dateCreated || new Date().toISOString(),
            normalization: "NFC",
        },
        idAuthorities: {
            wycliffeassociates: {
        ...
        languages: [
            {
                ...
                name: {
                    ...
                    ...(localeTag
                        ? { [localeTag]: args.draft.language.localName }
                        : {}),
                },
        ...
        localizedNames: normalizeLocalizedNamesText(
            args.draft.localizedNamesText,
        ),
        ingredients,
        copyright: {
