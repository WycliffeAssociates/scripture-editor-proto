...
function getBookTitle(
  ...
  if (metadata.localizedNames?.[`book-${bookCode.toLowerCase()}`]) {
    const nameObj = metadata.localizedNames[`book-${bookCode.toLowerCase()}`];
...
function mapBurritoIngredientsToFiles(
  ...
  for (const [filePath, ingredient] of Object.entries(metadata.ingredients)) {
    ...
    if (!bookCode) {
      console.warn(`Could not extract book code from ingredient: ${filePath}`);
      continue;
    }
...
export class ScriptureBurritoProjectLoader implements IProjectLoader {
  ...
  async loadProject(
    ...
    try {
      ...
      if (!metadata) {
        console.warn(`Invalid metadata.json structure: ${validationError}`);
        return null;
      }
      const defaultLanguageTag = metadata.meta.defaultLocale || "en";
      const defaultLanguageName =
        metadata.languages?.find((lang) => lang.tag === defaultLanguageTag)
      ...
      const project: ScriptureBurritoProject = {
        id:
          metadata.identification?.name[defaultLanguageTag] ?? projectDir.name,
        name:
          metadata.identification?.name[defaultLanguageTag] ?? projectDir.name,
        ...
        metadata: {
          ...
          name: metadata.identification?.name[defaultLanguageTag] ?? "",
          language: {
