...
function extractBookCodeFromIngredient(
  ...
  if (ingredient.scope && typeof ingredient.scope === "object") {
    const bookCodes = Object.keys(ingredient.scope);
    const validBookCode = bookCodes.find((code) => bookCodePattern.test(code));
...
export class ScriptureBurritoProjectLoader {
  ...
  async openResource(args: {
    ...
    try {
      ...
      const metadataItemType = classifyResourceKindFromScriptureBurrito({
        abbreviation:
          metadata.identification?.abbreviation?.[defaultLanguageTag] ??
      ...
      > = {
        ...
        projectId:
          metadata.identification?.name[defaultLanguageTag] ?? args.folderName,
        projectType:
        ...
        descriptor: {
          ...
          language: {
            ...
            direction: lang.scriptDirection === "rtl" ? "rtl" : "ltr",
          },
        ...
        const packedDocuments = packedTranslationNoteBookCodes.map(
          (bookCode) =>
            ({
              ...
              name: bookCode,
              browsePath: [bookCode],
            }) satisfies ReturnType<typeof createReferenceDocumentReference>,
        );
  ...
  async openProject(args: {
    ...
    try {
      ...
      const resolveIngredientPath = (storageKey: string): string | null => {
        for (const filePath of Object.keys(metadata.ingredients || {})) {
          const relativePath = removeLeadingDirSlashes(filePath);
          const fileName = relativePath.split("/").pop() || relativePath;
      ...
      const project: PathProject = {
        ...
        language: {
          ...
          name: defaultLanguageName,
          direction:
            defaultLanguageDirection === LanguageDirection.RTL ? "rtl" : "ltr",
        },
        books,
        ...
        getBook: async (storageKey) => {
          ...
          if (!bookCode || !ingredient) {
            throw new Error(`Invalid ingredient for storage key ${storageKey}`);
          }
          ...
          return {
            bookCode,
            title: getBookTitle(bookCode, metadata, defaultLanguageTag),
            fileName: storageKey,
            storageKey,
            path: `${args.projectRootPath}/${relativePath}`,
        ...
        saveBook: async (storageKey, usfmText) => {
          ...
          const bookCode = ingredient
            ? extractBookCodeFromIngredient(relativePath, ingredient)
            : undefined;
