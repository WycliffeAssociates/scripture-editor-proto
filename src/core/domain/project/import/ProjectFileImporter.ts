...
export class ProjectFileImporter implements Importer {
    ...
    public async importFile(zipFileHandle: IFileHandle): Promise<boolean> {
        ...
        try {
            ...
            await this.copyContentToFinalDestination(extractionResult.extractedTopLevelItem, finalProjectDir);

            console.log(`[FileProjectImporter] Project imported successfully to: ${finalProjectDir.path}`);
            return true;
     ...
     */
    private async getZipFileHandle(zipFilePath: string, tempDirectory: IDirectoryHandle): Promise<IFileHandle> {
        ...
        try {
            ...
            throw new Error(`[FileProjectImporter] Could not find ZIP file at path: ${zipFilePath}`);
        }
    }
     ...
     */
    private async extractZipToTemp(zipFileName: string, data: ArrayBuffer, tempDirectory: IDirectoryHandle): Promise<ExtractionResult> {
        // Create a unique temporary directory for this extraction
        const tempExtractionDirName = `${zipFileName.split(".")[0]}-extract-${Date.now()}`;
        ...
        console.log("[FileProjectImporter] Extraction to temporary directory complete.");
        ...
        if (topLevelEntries.length > 1) {
            console.warn(`[FileProjectImporter] Zip contains multiple top-level entries (${topLevelEntries.length}). Copying only the first one found: ${topLevelEntries[0].name}`);
        }
    ...
    private async resolveProjectDirectory(initialName: string, projectsDir: IDirectoryHandle): Promise<IDirectoryHandle> {
        ...
        console.log(`[FileProjectImporter] Final project directory created: ${finalProjectDir.path}`);
    ...
    private async copyContentToFinalDestination(sourceEntry: IPathHandle, destinationDir: IDirectoryHandle): Promise<void> {
        ...
        console.log("[FileProjectImporter] Copy to final project directory complete.");
    }
    ...
    private async copyFile(sourceFileHandle: IFileHandle, destinationDir: IDirectoryHandle, newFileName: string): Promise<void> {
        try {
            ...
            console.error(`[FileProjectImporter] Error copying file ${sourceFileHandle.name}:`, error);
        }
    }
     ...
     */
    private async cleanup(tempExtractionDir: IDirectoryHandle | null, zipFileHandle: IFileHandle | null): Promise<void> {
        const tempDirectory = await this.directoryProvider.tempDirectory;

        if (tempExtractionDir) {
            try {
                await tempDirectory.removeEntry(tempExtractionDir.name, { recursive: true });
                console.log(`[FileProjectImporter] Cleaned up temporary extraction directory: ${tempExtractionDir.name}`);
            } catch (e) {
                console.error("[FileProjectImporter] Error during cleanup of temporary extraction directory:", e);
            }
        }
        ...
        if (zipFileHandle) {
            try {
                ...
                await tempDirectory.removeEntry(zipFileHandle.name, { recursive: false });
                console.log(`[FileProjectImporter] Cleaned up original staged ZIP file: ${zipFileHandle.name}`);
            } catch (e) {
                console.error("[FileProjectImporter] Error during cleanup of original staged ZIP file:", e);
            }
        }
    }
}