...
export class WacsRepoImporter implements Importer {
    ...
    public async downloadData(
        ...
        console.log(
            `[WacsRepoImporter] Download successful. File size: ${data.byteLength} bytes.`,
        );
    ...
    private async extractZipToTemp(
        ...
        for (const fileName of Object.keys(loadedZip)) {
            ...
            if (fileName.endsWith("/")) {
                ...
                const fileHandle =
                    await currentExtractionTargetDir.getFileHandle(entryName, {
                        create: true,
                    });
        ...
        console.log(
            "[WacsRepoImporter] Extraction to temporary directory complete.",
        );
        ...
        if (topLevelEntries.length > 1) {
            console.warn(
                `[WacsRepoImporter] Zip contains multiple top-level entries (${topLevelEntries.length}). Copying only the first one found: ${topLevelEntries[0].name}`,
            );
        }
    ...
    private async copyContentToFinalDestination(
        ...
        console.log(
            "[WacsRepoImporter] Copy to final project directory complete.",
        );
    }
    ...
    private async copyFile(
        ...
        try {
            ...
            await writer.close();
            console.log(
                `[WacsRepoImporter] Wrote file: ${destFileHandle.path}`,
            );
        } catch (error) {
            console.error(
                `[WacsRepoImporter] Error copying file ${sourceFileHandle.name}:`,
                error,
            );
        }
    }
    ...
    private async cleanup(tempExtractionDir: IDirectoryHandle): Promise<void> {
        try {
            ...
            console.log(
                "[WacsRepoImporter] Temporary files and directories cleaned up.",
            );
        } catch (e) {
            // Log error but continue, as cleanup should not fail the main import process
            console.error(
                "[WacsRepoImporter] Error during cleanup of temporary files:",
                e,
            );
        }
    }
