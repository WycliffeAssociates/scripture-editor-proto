...
type BIBLE_BOOK_SLUG = (typeof BIBLE_ORDER)[number];
export const BIBLE_ENGLISH_VERSIFICATION_EXPECTED_CHAPTERS: Record<
  BIBLE_BOOK_SLUG,
  number
> = {
  GEN: 50,
...
export function parseReference(input: string) {
  ...
  const match = normalized.match(/^(\d?\s*[a-z]+)\s*(\d+)?(?:[:.\s](\d+))?$/i);
