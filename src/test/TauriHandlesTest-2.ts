...
import { TauriFileHandle } from "../persistence/TauriHandles";
...
describe("TauriFileHandle.createWritable()", () => {
    ...
    const executeWriteOps = async (handle: TauriFileHandle, ops: (writer: WritableStreamDefaultWriter) => Promise<void>) => {
    ...
    test("should seek and overwrite content correctly", async () => {
        ...
        expect(mockFileState[TEST_FILE_PATH]).toBe("The quick REDn fox jumps over the lazy dog.");
    });
    ...
    test("should process different data types correctly", async () => {
        ...
        expect(mockFileState[TEST_FILE_PATH]).toBe("String content. Hello and Blob");
    });
