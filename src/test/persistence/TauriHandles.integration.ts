...
vi.mock("@tauri-apps/plugin-fs", () => ({
    ...
    removeDir: vi.fn(async (path: string) => {
        ...
        const keysToDelete = Array.from(fileStore.keys()).filter((key) =>
            key.startsWith(path),
        );
...
describe.skipIf(!isTauriEnv)(
    "TauriFileHandle Integration Tests (LIVE FS I/O)",
    () => {
        ...
        afterAll(async () => {
            ...
            try {
                ...
                console.error(
                    `\t🚨 Failed final cleanup for ${testDirPath}: ${e}`,
                );
            }
        });
        ...
        integrationTest(
            "should write and append content using keepExistingData: true",
            async () => {
        ...
        integrationTest(
            "should perform a stream seek and overwrite content",
            async () => {
                const handle = new TauriFileHandle(testFilePath);
                ...
                const expected =
                    initialContent.slice(0, 10) +
                    "RED" +
                    initialContent.slice(13);
        ...
        integrationTest(
            "should truncate file and then seek/append correctly",
            async () => {
                const handle = new TauriFileHandle(testFilePath);
        ...
        integrationTest(
            "should perform default truncation (keepExistingData: false)",
            async () => {
                const handle = new TauriFileHandle(testFilePath);
                ...
                const writer = await handle
                    .createWritable()
                    .then((s) => s.getWriter()); // Note: No options passed
