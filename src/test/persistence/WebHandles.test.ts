...
describe('WebDirectoryHandleWrapper', () => {
    ...
    beforeEach(() => {
        ...
        mockResolveHandle = async (path: string): Promise<IPathHandle> => {
            ...
            for (let i = 0; i < parts.length; i++) {
                ...
                if (i === parts.length - 1) { // Last part, could be file or directory
                    try {
                        ...
                        return new WebFileHandleWrapper(fileHandle, currentPath, mockResolveHandle);
                    } catch {
                        const dirHandle = await currentNativeHandle.getDirectoryHandle(part);
                        return new WebDirectoryHandleWrapper(dirHandle, currentPath, mockResolveHandle);
                    }
            ...
            return new WebDirectoryHandleWrapper(mockRootDirectory, "/", mockResolveHandle);
        };
    });
    ...
    test('getDirectoryHandle creates and returns a new directory wrapper', async () => {
        const rootWrapper = new WebDirectoryHandleWrapper(mockRootDirectory, '/', mockResolveHandle);
        const childDir = await rootWrapper.getDirectoryHandle('newDir', { create: true });
        expect(childDir).toBeInstanceOf(WebDirectoryHandleWrapper);
    ...
    test('getFileHandle creates and returns a new file wrapper', async () => {
        ...
        expect(childFile).toBeInstanceOf(WebFileHandleWrapper);
    ...
    test('entries iterates through children and returns wrappers', async () => {
        ...
        expect(entries[0][1]).toBeInstanceOf(WebFileHandleWrapper);
        expect(entries[1][0]).toBe('dir1');
        expect(entries[1][1]).toBeInstanceOf(WebDirectoryHandleWrapper);
    });
    ...
    test('getParent returns the parent directory wrapper for a nested path', async () => {
        ...
        const level2Wrapper = new WebDirectoryHandleWrapper(
        ...
        expect(parent).toBeInstanceOf(WebDirectoryHandleWrapper);
    ...
    test('getParent returns the root directory for a top-level directory', async () => {
        const level1Wrapper = new WebDirectoryHandleWrapper(
...
describe('WebFileHandleWrapper', () => {
    ...
    test('constructor sets properties correctly for file', () => {
        ...
        const wrapper = new WebFileHandleWrapper(nativeHandle, '/root/testFile.txt', mockResolveHandle);
    ...
    test('getFile returns the underlying File object with correct content', async () => {
        ...
        const wrapper = new WebFileHandleWrapper(nativeHandle, '/test.txt', mockResolveHandle);
    ...
    test('getParent returns the parent directory wrapper', async () => {
        ...
        const fileWrapper = new WebFileHandleWrapper(
