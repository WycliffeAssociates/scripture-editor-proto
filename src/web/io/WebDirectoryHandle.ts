...
export class WebDirectoryHandle implements IDirectoryHandle {
  ...
  async getDirectoryHandle(
    ...
    const nativeParentHandle = await this._getOrCreateNativeDirectoryHandle(
      targetParentAbsolutePath,
      opts?.create || false
    );

    const childNativeHandle = await nativeParentHandle.getDirectoryHandle(
      targetBasename,
      opts
    );
  ...
  async getFileHandle(
    ...
    const fileNativeHandle = await nativeParentHandle.getFileHandle(
      targetBasename,
      opts
    );
  ...
  private async _getOrCreateNativeDirectoryHandle(
    ...
    for (const part of parts) {
      try {
        currentNativeHandle = await currentNativeHandle.getDirectoryHandle(
          part,
          {
            create: createIfNotFound,
          }
        );
      } catch (e) {
        if (createIfNotFound) {
