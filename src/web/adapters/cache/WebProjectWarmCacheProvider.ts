...
export class WebProjectWarmCacheProvider implements ProjectWarmCacheProvider {
    ...
    async read(projectPath: string): Promise<ProjectWarmCacheBlob | null> {
        try {
            const warmCacheDir = await this.getWarwmCacheDirectory();
            ...
            console.error("Error reading project warm cache:", error);
            return null;
        }
    }
    ...
    async clear(projectPath: string): Promise<void> {
        ...
        try {
            ...
            console.error("Error clearing project warm cache:", error);
