        ...
        const newProjectAdded = newProjects.find(
          (project) =>
            !initialProjects.find(
              (p) => p.projectDirectoryPath === project.projectDirectoryPath,
            ),
        );
        if (newProjectAdded) {
          router.navigate({
            to: `/$project`,
            params: { project: newProjectAdded.projectDirectoryPath },
            replace: true,
          });
        }
      } catch (err) {
        setError(
          err instanceof Error ? err.message : t`An unknown error occurred`, // Localize text
        );
        setProgressMessage("");
        ShowErrorNotification({
          notification: {
