...
export function buildGitRemoteProjectStatus(
  ...
  return {
    ...
    lastKnownLocalHead: patch.localHead ?? existingStatus.lastKnownLocalHead,
    lastKnownRemoteHead: patch.remoteHead ?? existingStatus.lastKnownRemoteHead,
    lastKnownLocalHeadAuthoredAt:
      patch.localHeadAuthoredAt ?? existingStatus.lastKnownLocalHeadAuthoredAt,
...
export function decideRemoteSyncAction(args: {
  ...
  const hasIncoming =
    args.status?.kind === GIT_REMOTE_PROJECT_STATUS_REMOTE_UPDATES_AVAILABLE ||
