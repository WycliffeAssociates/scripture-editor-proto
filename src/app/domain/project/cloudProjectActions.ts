...
const messages = {
  ...
  saveOwnCopyDuplicate: msg`You already have your own copy of this project. Connect to it instead.`,
  fallbackError: msg`Please try again.`,
};

/** Cloud action outcome — failures carry copy for the caller to show in-panel. */
...
export async function saveOwnCopyOnline(args: {
  ...
  try {
    ...
    // The common cause is a name collision: the user already forked this
    // upstream, so a second fork can't be created. Surface the friendlier
    // "connect to the one you have" copy in that case.
    const message = errorMessage(error, i18n);
    ...
    return {
      ...
      error: {
        ...
        message: isDuplicate ? i18n._(messages.saveOwnCopyDuplicate) : message,
      },
    };
  }
}
