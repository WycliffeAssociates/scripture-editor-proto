...
export class GitRemoteProjectService {
  ...
  async forkRepo(args: { owner: string; name: string; signal?: AbortSignal }) {
    const session = await this.requireSession();
  ...
  async attachProjectToRemote(args: {
    ...
    const session = await this.requireSession();
    const remoteMetadata = await this.remoteRepoProvider.inspectProjectMetadata(
      {
        hostBaseUrl: session.hostBaseUrl,
        token: session.token,
  ...
  private async requireSession() {
    ...
    if (!session) {
      throw new Error("Remote project operations require an active session");
    }
    return session;
  }
...
function assertRemoteRepoMatchesProject(args: {
  ...
  const remoteLanguage = normalizeMetadataTag(args.remoteMetadata.languageTag);
  if (!remoteLanguage) {
    throw new Error(
