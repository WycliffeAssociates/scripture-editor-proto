...
function serializeTokens(
  ...
  for (const token of tokens) {
    out.push(...serializeTokenToNodes(token, languageDirection, nestedEditors));
  }
  return out;
}
...
function serializeTokenToNodes(
  ...
  if (isNestedEditorMarker && !isClosingToken) {
    ...
    return [
      ...
      ...serializeTokens(token.content ?? [], languageDirection, nestedEditors),
    ];
  }
...
export function groupFlatNodesIntoParagraphContainers(
  ...
  for (let i = 0; i < flatNodes.length; i++) {
    ...
    if (!current) {
      ...
      current = startParagraph("p", `default-para-${paraIndex++}`, "", "\\p");
    }
...
function getContainerStartMarkerFromNode(
  ...
  const isUsfmTextNode =
    ...
    (node.type === "text" &&
      (node as { lexicalType?: string }).lexicalType === USFM_TEXT_NODE_TYPE);
