...
import {$getRoot, $setState, LexicalEditor, LexicalNode} from "lexical";
...
export function adjustSidsAsNeededOnTextTokens(editor: LexicalEditor) {
  return editor.registerNodeTransform(USFMTextNode, (node) => {
    ...
    if (nodeTokenType === UsfmTokenTypes.verseRange) {
      // recalc and change everything until next verseRange
      ...
      // const [_whole, bookCode, chapter, verseRange] = currentSidGroup;
      ...
      // set the verseRange node sid and all in between
    ...
    if (nodeTokenType === "text") {
      // make sure this node token type matches nearest previous verseRange (trimmed)
      let prevVerseRange: USFMTextNode | null = null;
      for (const prevNode of $reverseDfs(node, root)) {
        if (
          $isUSFMTextNode(prevNode.node) &&
          prevNode.node.getTokenType() === "verseRange"
        ) {
...
function findNextVerseRangeNode(
  ...
  for (const nextNode of $dfs(node, lastNode)) {
    if (
      ...
      nextNode.node.getTokenType() === "verseRange" &&
