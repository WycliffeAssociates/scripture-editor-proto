...
import {ParsedReference, parseSid} from "@/core/data/bible/bible";
...
function $handleVerseInsert({
  ...
  if (!isStartOfLine) {
    ...
    const alreadyHasVerseRangeSibling =
      ...
      sibling.getTokenType() === UsfmTokenTypes.verseRange;
...
function findContextForVerseInsert(anchorNode: LexicalNode): {
  ...
  for (const {node} of $reverseDfsIterator(anchorNode, $getRoot())) {
    if ($isUSFMTextNode(node)) {
      ...
      if (!prevSidInfo && tokenType === UsfmTokenTypes.verseRange) {
...
export function inverseTextNodeTransform({
  ...
  const undoableNodeTypes = [UsfmTokenTypes.marker, UsfmTokenTypes.verseRange];
