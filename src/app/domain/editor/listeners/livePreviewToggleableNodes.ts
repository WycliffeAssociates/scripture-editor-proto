import { $dfs, $reverseDfsIterator } from "@lexical/utils";
import {
    $getNodeByKey,
    $getRoot,
    $getSelection,
...
function collectBackwardToggleable(
    ...
    for (const prevNode of $reverseDfsIterator(node, root)) {
        ...
        if (TOKEN_TYPES_CAN_TOGGLE_HIDE.has(tType))
        ...
        for (const child of $dfs(parent, lastChild)) {
            ...
            if (TOKEN_TYPES_CAN_TOGGLE_HIDE.has(ctType))
