        ...
        return (
            <FormBlockCard
                ...
                onInsertBelow={(marker) => {
                    editor.update(() => {
                        if (marker === "v") {
                            ...
                            const anchorSid =
                                findLastVerseSidWalkingBackwards(live);
                            if (!anchorSid) return;
                        ...
                        const nextNode = new FormBlockNode(
                            newId,
                            this.__direction,
                            tokens,
                        );
                ...
                onDeleteFragment={(fragment: FormVerseFragment) => {
                    editor.update(() => {
                        const next = removeFragmentFromBlock(
                            this.__tokens,
                            fragment,
                        );
                        this.getWritable().__tokens = next;
                ...
                onInsertVerseBeforeFragment={(fragment) => {
                    editor.update(() => {
                        ...
                        const targetIdx = fragments.findIndex(
                            (f) => f.id === fragment.id,
                        );
                        const prevFragment =
                ...
                onSplitBeforeFragment={(fragment, marker) => {
                    editor.update(() => {
                        const split = splitBlockAtFragment(
                            this.__tokens,
                            fragment,
                            marker,
                        );
