...
export function NodeContextMenuPlugin() {
    ...
    () => [
      {
        ...
        type: "searchAction",
        doHide: () =>
          !suggestedSearchApiTerm || suggestedSearchApiTerm.length === 0,
        onSelect: () => {
  ...
  useEffect(() => {
    return editor.registerCommand(
      KEY_DOWN_COMMAND,
      (event: KeyboardEvent) => {
        const macCmdCombo = event.metaKey && event.key.toLowerCase() === "k";
        const winCtrlCombo = event.ctrlKey && event.key.toLowerCase() === "k";
      ...
      >
        ...
        <TextInput
          ...
          styles={{
            input: {
              border: "none",
              borderBottom: "1px solid var(--mantine-color-blue-filled)",
              borderRadius: 0,
              outline: "none",
              boxShadow: "none",
            },
