...
function findBestVisibleTarget(
  ...
  for (const candidate of anchorCandidates(finding)) {
    ...
    for (const match of directMatches) {
      ...
      if (visible && isRenderedElementCached(lookup.renderedState, visible)) {
        return adjustAnchorForMode(visible, isRawMode, lookup.renderedState);
      }
    }
...
function measureHighlightRects(
  ...
  for (const rect of domRects) {
    ...
    out.push({
      left: rect.left - rootRect.left + rootEl.scrollLeft - rootOffsetLeft,
...
export function FindingsOverlayPlugin({ editor }: FindingsOverlayPluginProps) {
  ...
  const shouldRender =
    rootEl !== null && entries.length > 0 && editorMode !== EDITOR_MODES.form;
          ...
          entry.kind === "highlight" ? (
            ...
            entry.rects.map((rect, i) => (
              <span
                ...
                data-content-finding={entry.annotationId ? "true" : undefined}
                data-annotation-id={entry.annotationId ?? undefined}
                style={{
                  left: `${rect.left}px`,
                  top: `${rect.top}px`,
                  width: `${rect.width}px`,
