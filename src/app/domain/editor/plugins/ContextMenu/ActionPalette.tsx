  ...
  return (
    <div>
      ...
      {group.actions.map((action) => (
        <ActionOptionRow key={action.id} action={action} context={context} />
      ))}
    </div>
  );
}
...
export function ActionPalette({ context, onClose }: ActionPaletteProps) {
  ...
  const handleSelectAction = (action: EditorAction) => {
              ...
              if (sid) {
                ...
                const findNodeBySid = (
                  ...
                  for (const node of nodeList) {
                    if ($isUSFMTextNode(node) && node.getSid() === sid) {
                      return node;
                    }
    ...
    if (nextStep && typeof nextStep === "object" && "onComplete" in nextStep) {
