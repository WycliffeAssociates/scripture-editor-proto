...
export function useEditorInput(editor: LexicalEditor) {
  ...
  useEffect(() => {
    ...
    const usfmAwarePasteUnregister = editor.registerCommand(
      ...
      (payload) => {
        const event =
          payload instanceof Event ? (payload as ClipboardEvent) : null;
        ...
        void (async () => {
          ...
          if (!parsed.ok) {
            showErrorNotification({
              notification: {
                title: "Paste Failed",
                message: "Invalid USFM content could not be pasted.",
              },
            });
            return;
          }
