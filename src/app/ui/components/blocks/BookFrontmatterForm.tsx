...
export function BookFrontmatterForm(props: BookFrontmatterFormProps) {
  ...
  function replaceEntry(
    ...
    commit(
      entries.map((entry) => (entry.id === targetId ? updater(entry) : entry)),
    );
  }
      ...
      <div className={styles.header}>
        <div className={styles.titleBlock}>
          ...
          <div className={styles.subtitle}>
            <Trans>Chapter 0 metadata and introductory markers</Trans>
          </div>
        </div>
      </div>
        ...
        {entries.map((entry, index) => (
          <div key={entry.id}>
            <FrontmatterCard
              ...
              onChange={(nextEntry) => replaceEntry(entry.id, () => nextEntry)}
              onControlPress={handleControlPress}
            />
            <InsertSlot
              insertIndex={index + 1}
            ...
            <Menu.Popup className={styles.insertMenuPopup}>
              <div className={styles.insertMenuList}>
                {ADDABLE_MARKERS.map((marker) => {
                  if (marker === "id" && props.hasId) return null;
                  ...
                  return (
                    ...
                    >
                      <span>{getLocalizedUsfmMarkerLabel(marker)}</span>
                      <span
    ...
    <article className={styles.card}>
      <div className={styles.cardHeader}>
        <div className={styles.cardTitleBlock}>
          ...
          <div className={styles.cardMarker}>{`\\${props.entry.marker}`}</div>
          {description ? (
      ...
      {props.entry.kind === "ide" ? (
        <>
          ...
          <div className={styles.note}>
            <Trans>
              Most projects should stay UTF-8. Change this only when the source
              text truly depends on another encoding.
            </Trans>
          </div>
          ...
          props.entry.encoding.toUpperCase() !== "UTF-8" ? (
            <div className={styles.warning}>
              <Trans>
                This file is not using UTF-8. Avoid changing the encoding
                casually because it can corrupt the text.
              </Trans>
            </div>
          ) : null}
        </>
