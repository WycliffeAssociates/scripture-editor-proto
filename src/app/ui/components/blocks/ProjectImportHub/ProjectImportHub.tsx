...
export function ProjectImportHub(props: ProjectImportHubProps) {
  ...
  const loadCatalogRepos = useCallback(async () => {
    ...
    try {
      ...
      setCatalogError(
        error instanceof Error ? error.message : t`Failed to fetch projects`,
      );
    } finally {
    ...
    >
      <header className={styles.header}>
        <div className={styles.headerCopy}>
          ...
          <p className={styles.description}>
            <Trans>
              Import a linked cloud project, download an unlinked copy, or
              import a local folder or ZIP.
            </Trans>
          </p>
          <p className={styles.sourceHint}>
            <Trans>
              Linked cloud projects import with their Gitea connection so
              changes can sync back. Download-only projects copy files onto this
              device and start unlinked.
            </Trans>
          </p>
          <p className={styles.sourceHint}>
            <Trans>
              Some projects may appear in both lists. Linked cloud keeps the
              Gitea connection. Download only imports a separate local copy.
            </Trans>
          </p>
        ...
        <div className={styles.searchField}>
          ...
          <input
            ...
            onChange={(event) => {
              ...
              gitea.setQuery(next);
              if (sourceFilter !== "cloud" && next.trim().length > 0) {
                void loadCatalogRepos();
              }
            }}
            ...
            <>
              <label className={styles.checkboxPill}>
                <input
                  ...
                  onChange={(event) =>
                    gitea.setScope(
                      event.currentTarget.checked ? "owned" : "all",
                    )
                  }
              ...
              <Button
                ...
                disabled={
                  gitea.isLoading || props.isImporting || props.isDisconnecting
                }
              >
                <Trans>Refresh</Trans>
              </Button>
          ...
          <tbody className={styles.tbody}>
            {showCloudLogin ? (
                ...
                <td className={styles.td} colSpan={3}>
                  <div className={styles.loginPanel}>
                    <p className={styles.loginCopy}>
                      {props.hostBaseUrl ? (
                        <Trans>
                          Connect to {props.hostBaseUrl}
                          to browse remote projects you can push and pull.
                        </Trans>
                      ) : (
                        <Trans>
                          Remote login is not configured for this build yet.
                        </Trans>
                      )}
                    </p>
                    {props.hostBaseUrl ? (
              ...
              <tr>
                <td className={styles.td} colSpan={3}>
                  <div className={styles.emptyState}>
                    <Trans>Loading downloadable projects...</Trans>
                  </div>
                </td>
              </tr>
            ) : sourceFilter === "cloud" &&
              ...
              <tr>
                <td className={styles.td} colSpan={3}>
                  <div className={styles.emptyState}>
                    <Trans>Please enter a more specific search term.</Trans>
                  </div>
                </td>
              </tr>
            ) : sourceFilter === "cloud" &&
              ...
              <tr>
                <td className={styles.td} colSpan={3}>
                  <div className={styles.emptyState}>
                    <div>
                      <Trans>
                        No linked cloud projects matched this search.
                      </Trans>
                    </div>
                    <div>
                      <Trans>
                        Your search returned results, but none were cloud
                        projects for this app. Try a more specific search term.
                      </Trans>
                    </div>
                  </div>
                </td>
                ...
                <td className={styles.td} colSpan={3}>
                  <div className={styles.emptyState}>
                    {gitea.isSearchMode ? (
                      ...
                      <Trans>Loading remote projects...</Trans>
                    )}
                  </div>
                </td>
              </tr>
                      ...
                      props.sessionUsername ? (
                        gitea.isLowSignalSearch ? (
                          ...
                          <>
                            ...
                            <div>
                              <Trans>
                                Your search returned results, but none were
                                cloud projects for this app. Try a more specific
                                search term.
                              </Trans>
                            </div>
                          </>
                          ...
                          <Trans>
                            No repos you own are visible in this list yet.
                          </Trans>
                        ) : (
                          <Trans>
                            No remote projects are available for this account
                            yet.
                          </Trans>
                        )
                      ) : (
                        <Trans>Select a source to continue.</Trans>
                      )
                    ) : normalizedSearch.length > 0 ? (
                      <Trans>No matching projects</Trans>
                    ) : (
                      <Trans>
                        Search a language, title, owner, or repo name to see
                        projects.
                      </Trans>
                    )}
                  </div>
              ...
              rows.map((row) => (
                <tr key={row.id} className={styles.tbodyRow}>
                  <td className={styles.td}>
                    <span className={styles.projectCell}>{row.title}</span>
                  </td>
                  <td className={styles.td}>
                  ...
                  <td className={styles.td}>
                    <Button
                      ...
                      onClick={() => {
                        if (row.kind === "catalog") {
                          void downloadCatalogRepo(row.repo);
                        } else {
                    ...
                    >
                      {row.kind === "catalog" ? (
                        downloadingRepoId === row.id ? (
                          ...
                          <Trans>Download local copy</Trans>
                        )
                      ) : (
                        <Trans>Import linked copy</Trans>
                      )}
                    </Button>
                  </td>
                </tr>
      ...
      {shouldShowCloudPagingHint({
        ...
        <div className={styles.footerActions}>
          <span className={styles.sourceHint}>
            <Trans>
              Showing initial {gitea.visiblePageSize} linked cloud projects.
              More results available via search.
            </Trans>
          </span>
        </div>
      ) : null}
