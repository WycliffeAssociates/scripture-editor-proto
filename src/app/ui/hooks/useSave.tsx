...
export function useProjectDiffs({
  ...
  if (!currentSidMap.current || !originalSidMapRef.current) {
    console.log("Calculating CURRENT SID map... (This should only run once!)");
  ...
  function toggleDiffModal(saveCurrentDirtyLexical: () => void) {
    if (openDiffModal) {
      ...
      if (pickedFile && pickedChapter) {
        updateDiffMapForChapter(pickedFile.bookCode, pickedChapter.chapNumber);
  ...
  async function saveProjectToDisk() {
    ...
    const uniqueBookIdsWithDiff = Object.keys(diffMap).reduce((acc, key) => {
...
export function getProjectSidContentMap(
  ...
  for (const file of workingFiles) {
    ...
    for (const chapter of file.chapters) {
      ...
      Object.assign(projectMap[file.bookCode][chapter.chapNumber], chapterMap);
    }
  }
  ...
  for (const change of sequenceChanges) {
    if (change.added) {
      for (const uniqueKey of change.value) {
        ...
        if (!cur) {
          throw new Error(`Missing current content for key: ${uniqueKey}`);
        }
      ...
      for (const uniqueKey of change.value) {
        ...
        if (!original) {
          throw new Error(`Missing original content for key: ${uniqueKey}`);
        }
        chapterDiffs[uniqueKey] = buildRichDiff(
          uniqueKey,
          original,
...
function mutateWorkingFilesRefRevertChange(
  ...
  if (isAddition && diff.current) {
    const { parentChapterNodeList, startIndexInParent, nodes } = diff.current;
