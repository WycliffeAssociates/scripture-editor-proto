...
export function useNavigation({
  ...
  function switchBookOrChapter(fileBibleIdentifier: string, chapter: number) {
    ...
    if (!chapterState) {
      ...
      if (targetFile.chapters.length > 0) {
        const sortedChaps = targetFile.chapters
          .slice()
          .sort((a, b) => a.chapterNumber - b.chapterNumber);
        if (chapter > sortedChaps[sortedChaps.length - 1].chapterNumber) {
  ...
  const determineNextChapter = () => {
    ...
    if (currentIndex === pickedFile.chapters.length - 1) {
      ...
      const nextChap = pickedFile.chapters[currentIndex + 1].chapterNumber;
      return {
        hasNext: true,
  ...
  const determinePrevChapter = () => {
    ...
    if (currentIndex === 0) {
      ...
      const prevChapter = pickedFile.chapters[currentIndex - 1].chapterNumber;
      return {
        hasPrev: true,
          ...
          editor.read(() => {
            ...
            const findNodeBySid = (
              ...
              for (const node of nodes) {
                if ($isUSFMTextNode(node) && node.getSid() === verseSid) {
                  return node;
                }
                if ($isElementNode(node)) {
                  const found = findNodeBySid(node.getChildren());
            ...
            if (targetNode) {
              const domEl = editor.getElementByKey(targetNode.getKey());
              if (domEl) {
