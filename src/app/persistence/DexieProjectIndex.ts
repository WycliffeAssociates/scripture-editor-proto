...
export class DexieProjectIndex implements ProjectIndex {
  ...
  async listLibraryItems(): Promise<ResourceLibraryItem[]> {
    ...
    return projects.map((project) =>
      dbProjectToLibraryItem({
        project,
        language: languages.find((lang) => lang.id === project.languageId),
      }),
    );
  }

  async getProjectByPath(projectPath: string): Promise<ProjectListItem | null> {
  ...
  async indexItem(item: LibraryItem | Project): Promise<void> {
    ...
    const hasRemoteSync = "type" in item ? isRemoteSyncCapable(item) : false;
    const projectPath =
    ...
    await this.db.transaction(
      ...
      async () => {
        ...
        if (existingProject) {
          await this.db.projects.update(existingProject.id, projectData);
        } else {
        ...
        await this.db.files.where("projectId").equals(projectRow.id).delete();
        ...
        for (const book of books) {
          ...
          const fileExtension =
            extensionStart >= 0 ? book.path.substring(extensionStart) : null;
  ...
  async deleteProject(projectPath: string): Promise<void> {
    await this.db.transaction(
      ...
      async () => {
        ...
        await this.db.files.where("projectId").equals(project.id).delete();
