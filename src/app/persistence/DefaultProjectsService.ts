...
export class DefaultProjectsService implements ProjectsService {
  ...
  async readProjectOrigin(projectPath: string): Promise<ProjectOrigin | null> {
  ...
  private async loadProject(projectRef: string): Promise<FacadeProject | null> {
    try {
  ...
  private async loadResource(
    ...
    try {
      ...
      const indexed = await this.projectIndex.getLibraryItemByPath(projectPath);
      console.debug(
      ...
      const resource = await this.reopenManagedResource({
        managedPath: projectPath,
        displayName: indexed?.displayName ?? basenameStoragePath(projectPath),
      });
      return resource
  ...
  async openEditableProject(
    ...
    try {
      ...
      const displayName = await this.resolveProjectDisplayName(projectPath);
      ...
      console.error(`Error opening editable project ${projectRef}:`, error);
      return { project: null, rejectionReason: "not-found" };
    }
  }
  ...
  async openProjectReadOnly(projectRef: string): Promise<FacadeProject | null> {
    return this.loadProject(projectRef);
  }

  async openResource(projectRef: string): Promise<LoadedReferenceItem | null> {
    ...
    try {
      ...
      if (!loadedProject) {
        if (editableResult.rejectionReason === "metadata-invalid") {
          const metadataDocument = await this.loadMetadataEditor(importedPath, {
            includeIssues: true,
          });
          if (!metadataDocument) {
            throw new Error(
          ...
          const metadataReviewWorkspace = buildMetadataReviewWorkspace({
        ...
        if (!loadedResource) {
          throw new Error(`Imported project could not be loaded: ${projectId}`);
        }
        ...
        const indexedResource =
          (await this.projectIndex.getLibraryItemByPath(importedPath)) ??
      ...
      try {
        ...
        await this.reportImportProgress(
          options,
          createImportProgressUpdate(ImportProgressPhase.FAILED, warning),
        );
        return {
          project: indexedProject,
          gitReady: false,
  ...
  async createRemoteForProject(projectRef: string): Promise<{
    ...
    if (publishResult.kind !== PUBLISH_AFTER_SAVE_PUBLISHED) {
      throw new Error(describeInitialRemotePublishFailure(publishResult.kind));
    }
    return result;
  }
  ...
  async cloneWritableRemoteProject(
    ...
    try {
      const cloneResult = await remoteService.cloneRemoteRepoToManagedPath({
        projectPath,
        repo: args.repo,
        gitProvider: this.gitProvider,
      });
  ...
  async reconcileIndex(): Promise<void> {
    ...
    const projectEntries = await this.fileSystem.list(this.roots.projectsRoot);
  ...
  async readDeclaredSources(workspacePath: string): Promise<DeclaredSource[]> {
  ...
  private async cleanupFailedClonedProject(projectPath: string): Promise<void> {
    if (await this.fileSystem.exists(projectPath)) {
