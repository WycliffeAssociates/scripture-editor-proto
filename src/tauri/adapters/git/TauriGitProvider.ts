...
export class TauriGitProvider implements GitProvider {
  ...
  async listHistory(
    ...
    return history.map((entry) => {
      ...
      return {
        ...
        authoredAtIso: new Date(entry.authored_at_unix * 1000).toISOString(),
        subject: entry.subject,
  ...
  async readCommitDetails(
    ...
    const entry = await invoke<TauriHistoryEntry>("git_read_commit_details", {
      repoPath: projectPath,
      commitHash,
    });
    return {
  ...
  async fetchRemoteHeads(args: {
    ...
    const raw = await invoke<TauriRemoteInspection>("git_fetch_remote_heads", {
      repoPath: args.projectPath,
      remoteName: args.remoteName,
      branch: args.branch,
      username: args.auth.username,
