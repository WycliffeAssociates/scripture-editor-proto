  ...
  useLayoutEffect(() => {
    return editor.registerUpdateListener(
      ({editorState, dirtyElements, dirtyLeaves, prevEditorState, tags}) => {
        ...
        if (tagsToIgnore && [...tags].some((t) => tagsToIgnore.has(t))) {
          return;
        }

        // bail if just a history merge tag
        if (tags.has(HISTORY_MERGE_TAG) && ignoreHistoryMergeTagChange) {
          return;
        }
        ...
        const hasDirties = dirtyElements.size > 0 || dirtyLeaves.size > 0;
