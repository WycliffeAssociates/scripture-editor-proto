...
export const pushTokenContentOntoMarker = (
    ...
    const makeIntermediate = (
        ...
        const it: IntermediateToken = {
            type: token.type ?? "",
            value: valueIsTextList.has(token.type ?? "")
                ? token.text
                : token.value,
        };
        if (prevMarker) {
    ...
    const flattened: IntermediateToken[] = nlSeparatedChunks.flatMap(
        (chunk) => [...processChunk(chunk), { type: "nl", value: "\n" }],
    );
    return flattened;
};
